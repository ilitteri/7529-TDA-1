...
class Grafo:
	...
	def actualizar_peso(self, v: str, w: str, peso) -> None:
		self.borrar_arista(v, w)
		self.agregar_arista(v, w, peso)
	...
	def accesibles_desde(self, v:str) -> list:
		visitados = []
		cola = [v]
		while len(cola) > 0:
			actual = cola.pop(0)
			if actual not in visitados:
				...
				for vecinos in self.adyacentes_a(actual):
					if self.peso_de_arista_entre(actual, vecinos) != 0:
						cola.append(vecinos)
		return visitados